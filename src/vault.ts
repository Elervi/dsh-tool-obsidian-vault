 ...
 */
export function injectedVaultPath(): string | undefined {
...
export function injectedVaultName(): string | undefined {
...
export function applyFrontmatterUpdate(
  ...
  for (const [k, v] of setEntries) {
    if (/\n/.test(v)) throw new VaultError(`frontmatter 值必须单行（字段 ${k} 的取值含换行）；列表请用内联数组 [a, b]`, VaultCode.FRONTMATTER_MULTILINE)
  }
