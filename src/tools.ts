...
import { bridgeFor, markerBridge, type BridgeClient } from './bridge.js'
import {
  ...
  splitListValue, mapLimit, indexAliases, injectedVaultPath, parseLinkBody,
} from './vault.js'
 ...
 */
function bridgeOrNull(config: VaultConfig, root: string, signal?: AbortSignal): Promise<BridgeClient | null> {
  if (!config.bridge) return Promise.resolve(null)
  return bridgeFor(root, signal)
}

/** 按行切片（vault_read_note 桥/文件两路共用） */
function sliceNote(content: string, offset?: number, limit?: number): {
  content: string
  totalLines: number
  from: number
  to: number
 ...
 */
async function observeBridgeWrite(
  ctx: Context,
  fs: FileSystem,
  root: string,
  rel: string,
  ...
  try {
    ...
    if (info && info.type === 'file') emitObserved(ctx, target, info.version, exec)
  } catch {
    // best-effort
  }
}
...
export function registerTools(ctx: Context, config: VaultConfig): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute() {
      ...
      let bridgeCurrentPath: string | undefined
      if (config.bridge) {
        const injected = injectedVaultPath()
        if (injected) {
          const probe = await bridgeFor(injected).catch(() => null)
          if (probe) bridgeCurrentPath = injected
        }
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(_args, exec) {
      ...
      const root = await resolveVaultRoot(config, exec as CwdExec)
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (bridge) {
        ...
        if (read.size !== undefined) result.bytes = read.size
        return result
      }
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (bridge) {
        ...
        await observeBridgeWrite(ctx, fs, root, res.path, exec)
        return { path: rel, operation: 'append' as const, addedChars: res.addedChars, ...(res.bytes !== undefined ? { bytes: res.bytes } : {}) }
      }
      const target = await resolveNoteTarget(fs, root, rel, config.allowSymlinkEscape)
      const info = await fs.stat(target, exec.signal)
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (bridge) {
        ...
        if (meta.size !== undefined) result.bytes = meta.size
        return result
      }
      const target = await resolveNoteTarget(fs, root, rel, config.allowSymlinkEscape)
      const info = await fs.stat(target, exec.signal)
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        const v = value as { path: string; trashed: boolean }
    ...
    presentCall: (args) => ({ card: 'generic', title: `删除笔记 ${(args as { path: string }).path}（回收站）` }),
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        const v = value as { path: string; opened: boolean }
    ...
    presentCall: (args) => ({ card: 'generic', title: `在 Obsidian 中打开 ${(args as { path: string }).path}` }),
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        const v = value as { total: number; tags: Array<{ tag: string; count: number }> }
    ...
    async execute(args, exec) {
      ...
      await mapLimit(notes, 8, async (note) => {
        try {
          ...
          for (const t of extractTags(body)) counts.set(t, (counts.get(t) ?? 0) + 1)
        } catch {
          // 读取失败跳过
        }
      })
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        const v = value as { path: string; link: string; format: string }
    ...
    async execute(args, exec) {
      ...
      return { path: rel, link, format: 'wikilink' as const }
    },
    presentCall: (args) => ({ card: 'generic', title: `生成链接 ${(args as { path: string }).path}` }),
  }))
}
