...
async function listFiles(dir: string): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      for (const child of await listFiles(full)) out.push(join(entry.name, child))
    } else {
      out.push(entry.name)
    }
  }
...
async function hashTree(dir: string): Promise<Record<string, string>> {
  ...
  for (const rel of await listFiles(dir)) map[rel] = await hashFile(join(dir, rel))
  return map
}
...
async function readManifest(target: string): Promise<Record<string, string>> {
  try {
    ...
    if (parsed !== null && typeof parsed === 'object' && !Array.isArray(parsed)) return parsed as Record<string, string>
  } catch {
    /* 无清单（首装前 / 历史遗留） */
  }
  return {}
...
async function writeManifest(target: string, map: Record<string, string>): Promise<void> {
  await writeFile(join(target, MANIFEST), `${JSON.stringify(map, null, 2)}\n`, 'utf8')
}

/** 递归拷贝一个文件或目录到目标，确保父目录存在。 */
async function copyTo(sourcePath: string, destPath: string): Promise<void> {
...
export async function installPreset(options: InstallPresetOptions = {}): Promise<InstallPresetResult> {
  ...
  for (const [rel, bHash] of Object.entries(manifest)) {
    if (theirs[rel] !== undefined) continue // 上游仍在
    const dst = join(target, rel)
