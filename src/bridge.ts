...
export function bridgeMarkerPath(): string {
...
function mergeSignal(signal: AbortSignal | undefined, timeoutMs = 30_000): { signal: AbortSignal; cleanup: () => void } {
...
export class BridgeClient {
  ...
  private async request<T>(
    method: 'GET' | 'POST',
    urlPath: string,
    opts: { query?: Record<string, string | number | boolean | undefined>; body?: unknown; signal?: AbortSignal } = {},
  ): Promise<T> {
    ...
    try {
      ...
      throw new VaultError(
        `Obsidian 桥不可用：${err instanceof Error ? err.message : String(err)}（若 Obsidian 或 DSH Dock 未运行，vault 工具已自动回退文件直读模式）`,
        'BRIDGE_UNAVAILABLE',
      )
    } finally {
      cleanup()
  ...
  async health(): Promise<{ ok: boolean; vault?: { name?: string; path?: string } }> {
    return this.request('GET', '/health')
  }

  async current(): Promise<{ name: string; path: string; activeFile?: string; updatedAt: number }> {
    return this.request('GET', '/v1/current')
  }

  async listNotes(opts: { folder?: string; all?: boolean; ignoreDirs: string[] }): Promise<{
    total: number
    notes: Array<{ path: string; size?: number; extension?: string }>
  }> {
  ...
  async listFolders(opts: { folder?: string; ignoreDirs: string[] }): Promise<{
    total: number
  ...
  async metadata(rel: string): Promise<{
    path: string
    size?: number
    mtime?: number
    ...
    tags: string[]
    aliases: string[]
    wikilinks: Array<{ body: string; embedded: boolean }>
    markdown: Array<{ target: string; text: string }>
    unresolved: number
  }> {
    return this.request('GET', '/v1/metadata', { query: { path: rel } })
  }

  async backlinks(req: { path?: string; title?: string; format?: 'wikilink' | 'markdown' | 'all' }): Promise<{
    total: number
    backlinks: BridgeHit[]
    target?: string
    ambiguous?: boolean
  ...
  async search(opts: {
    ...
    match_all?: boolean
    ignoreDirs: string[]
  }): Promise<{ total: number; hits: BridgeHit[] }> {
  ...
  async searchTags(opts: { tag: string; folder?: string; limit?: number; ignoreDirs: string[] }): Promise<{
    total: number
    hits: Array<{ path: string; tags: string[] }>
  }> {
  ...
  async frontmatter(rel: string): Promise<{
    path: string
    present: boolean
    valid: boolean
  ...
  async writeNote(req: {
    ...
    overwrite?: boolean
    unique?: boolean
  }): Promise<{ path: string; operation: 'create' | 'update'; bytes?: number }> {
    return this.request('POST', '/v1/write', { body: { ...req, op: 'write' } })
  }

  async appendNote(rel: string, content: string): Promise<{ path: string; operation: 'append'; addedChars: number; bytes?: number }> {
  ...
  async editNote(req: {
    ...
    new_string: string
    replace_all?: boolean
  }): Promise<{ path: string; before: string; after: string; matches: number }> {
  ...
  async updateFrontmatter(req: {
    ...
    path: string
    created: boolean
    changes: Array<{ op: 'set' | 'delete'; key: string; value?: string }>
  ...
  /** 回收站删除（可恢复） */
  async trashNote(rel: string): Promise<{ path: string; trashed: true }> {
    return this.request('POST', '/v1/trash', { body: { path: rel } })
  }

  /** 在 Obsidian 中打开/聚焦笔记 */
  async openNote(rel: string): Promise<{ path: string; opened: true }> {
    return this.request('POST', '/v1/open', { body: { path: rel } })
  }

  /** 全库标签聚合（含计数） */
  async allTags(opts: { folder?: string; ignoreDirs: string[] }): Promise<{
    total: number
  ...
  /** 生成指向笔记的标准链接文本 */
  async noteLink(rel: string, source?: string): Promise<{ path: string; link: string; format: 'wikilink' | 'markdown' }> {
...
let healthCache: { root: string; bridge: BridgeClient | null; at: number } | undefined
 ...
 */
export async function bridgeFor(root: string, signal?: AbortSignal): Promise<BridgeClient | null> {
  const env = bridgeEnv() ?? (await markerBridge(root))
  if (!env) return null
  const now = Date.now()
  const hit = healthCache
  if (hit && hit.root === root && now - hit.at < HEALTH_TTL_MS) return hit.bridge
  let bridge: BridgeClient | null = null
  try {
    const client = new BridgeClient(env.url, env.token)
    const health = await client.health()
    const norm = (p?: string) => (p ?? '').replace(/[\\/]+$/, '')
    if (health?.ok && norm(health.vault?.path) === norm(root)) bridge = client
  } catch {
    bridge = null
  }
