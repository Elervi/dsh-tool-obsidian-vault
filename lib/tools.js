 ...
 * no residue.
 */
async function rollbackRewrites(ctx, fs, planned, committed, exec) {
    const errors = [];
 ...
 */
async function resolveVaultRoot(config, exec, vaultArg) {
    ...
    if (vaultArg && vaultArg.trim().length > 0) {
        ...
        if (byName)
            return norm(byName.path);
        const looksAbsolute = target.startsWith('/') || /^[A-Za-z]:[\\/]/.test(target);
        if (!looksAbsolute)
            throw new VaultError(`未知的 Obsidian 库：${target}（可用 vault_list_vaults 查看，或传绝对路径）`, VaultCode.UNKNOWN_VAULT);
        if (!config.allowArbitraryRoots) {
            throw new VaultError(`未注册的库路径：${target}（默认只允许已发现的库与 vaultRoots；如需放行任意绝对路径，请配置 allowArbitraryRoots: true）`, VaultCode.ROOT_UNREGISTERED);
        }
        return norm(target);
    }
    if (config.vaultRoot && config.vaultRoot.length > 0)
        return norm(config.vaultRoot);
 ...
 */
async function resolveCurrentVault(config, exec) {
    const discovered = config.discoverVaults ? await discoverVaults() : [];
    const norm = (p) => p.replace(/\/+$/, '');
    const cwd = exec.agent?.session?.header?.cwd;
    if (config.vaultRoot && config.vaultRoot.length > 0) {
    ...
    if (injected) {
        const hit = discovered.find((v) => norm(v.path) === norm(injected));
        if (hit)
            return { name: hit.name, path: norm(hit.path), source: 'dsh-dock 注入的本服务所属库（per-vault 隔离）' };
    }
    if (typeof cwd === 'string' && cwd.length > 0) {
        const hit = discovered.find((v) => norm(v.path) === norm(cwd));
        if (hit)
            return { name: hit.name, path: norm(hit.path), source: '会话工作目录恰好是库' };
    }
 ...
 */
function noteRelPath(input) {
    const trimmed = input.trim();
    if (trimmed === '')
        throw new VaultError('笔记路径不能为空', VaultCode.PATH_INVALID);
 ...
 */
async function resolveNoteTarget(fs, root, rel, allowSymlinkEscape) {
...
async function vaultNotes(fs, config, root, exec, includeAll = false) {
    const rootTarget = await fs.resolve(root, { cwd: root });
...
export function registerTools(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            return {
                total: notes.length,
                notes: sliced.map((n) => {
                    const item = { path: n.path };
                    if (n.size !== undefined)
                        item.size = n.size;
                    if (a.all && n.extension !== undefined)
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: (args) => ({ card: 'generic', title: `在 vault 搜索“${args.query}”` }),
    }));
    ctx.tools.register(defineTool({
        ...
        presentCall: (args) => ({ card: 'generic', title: `搜索 #${args.tag} 标签` }),
    }));
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const info = await fs.stat(target, exec.signal);
            if (!info)
                throw new VaultError(`笔记不存在：${rel}`, VaultCode.NOTE_NOT_FOUND);
            if (info.type !== 'file')
        ...
        presentCall: (args) => ({ card: 'generic', title: `读取笔记 ${args.path}` }),
    }));
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (info && info.type !== 'file')
                throw new VaultError(`路径已存在但不是文件：${rel}`, VaultCode.NOT_FILE);
            if (info && a.unique) {
        ...
        presentCall: (args) => ({ card: 'generic', title: `写入笔记 ${args.path}` }),
    }));
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (info.type !== 'file')
                throw new VaultError(`路径不是文件：${rel}`, VaultCode.NOT_FILE);
            try {
                const outcome = await fs.editText(target, { oldString: a.old_string, newString: a.new_string, replaceAll: Boolean(a.replace_all) }, { version: info.version }, exec.signal);
                emitObserved(ctx, target, outcome.version, exec);
        ...
        presentCall: (args) => ({ card: 'generic', title: `编辑笔记 ${args.path}` }),
    }));
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const info = await fs.stat(target, exec.signal);
            if (!info)
                throw new VaultError(`笔记不存在：${rel}（如需新建请用 vault_create_note）`, VaultCode.NOTE_NOT_FOUND);
            if (info.type !== 'file')
                throw new VaultError(`路径不是文件：${rel}`, VaultCode.NOT_FILE);
            try {
        ...
        presentCall: (args) => ({ card: 'generic', title: `追加内容到 ${args.path}` }),
    }));
    ctx.tools.register(defineTool({
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                const v = value;
                if (!v.present)
                    return [{ type: 'text', text: `${v.path} 没有 frontmatter（不以 --- 开头）` }];
        ...
        presentCall: (args) => ({ card: 'generic', title: `读取 frontmatter ${args.path}` }),
    }));
    ctx.tools.register(defineTool({
        ...
        presentCall: (args) => ({ card: 'generic', title: `更新 frontmatter ${args.path}` }),
    }));
    ctx.tools.register(defineTool({
        ...
        presentCall: (args) => ({ card: 'generic', title: `列出出链 ${args.path}` }),
    }));
    ctx.tools.register(defineTool({
            ...
            try {
                ...
                for (const link of extractLinks(content)) {
                    if (link.embedded)
                        ...
                        wikilinks++;
                    if (!resolveLinkTarget(resolver, link.target, link.stem))
                        unresolved++;
                }
                ...
                for (const md of mdLinks) {
                    markdown++;
                    if (!resolveMarkdownTarget(md.target, noteDir, resolver))
                        unresolved++;
                }
        ...
        presentCall: (args) => ({ card: 'generic', title: `笔记信息 ${args.path}` }),
    }));
    ctx.tools.register(defineTool({
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (oldRel === newRel)
                throw new VaultError('新旧路径相同，无需重命名', VaultCode.INVALID_ARGS);
            ...
            if (!oldInfo)
                throw new VaultError(`笔记不存在：${oldRel}`, VaultCode.NOTE_NOT_FOUND);
            if (oldInfo.type !== 'file')
                throw new VaultError(`路径不是文件：${oldRel}`, VaultCode.NOT_FILE);
            const newInfo = await fs.stat(newTarget, exec.signal);
            if (newInfo)
                throw new VaultError(`目标已存在：${newRel}`, VaultCode.EXISTS);
            let content;
            try {
