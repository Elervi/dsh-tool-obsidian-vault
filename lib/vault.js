...
function yamlScalar(value) {
    ...
    if (trimmed !== value)
        return quoteYaml(value); // 首尾空白会改变词法
    if ((trimmed.startsWith('[') && trimmed.endsWith(']')) || (trimmed.startsWith('{') && trimmed.endsWith('}'))) {
        return value; // 内联数组 / 映射：保持流式结构（文档契约）
    }
    if (/^[-?:,[\]{}#&*!|>'"%@`]/.test(trimmed))
        return quoteYaml(value); // 特殊前导字符（含注释 / 列表项 / 引号起始）
    if (/:\s|:\s*$/.test(value))
        return quoteYaml(value); // 冒号后空白 / 行尾冒号 → 非法或歧义
    if (/,\s|\s#/.test(value))
        return quoteYaml(value); // 逗号+空格（流式序列歧义）、注释起始
    if (value.includes('"') || value.includes("'"))
        return quoteYaml(value); // 裸引号
    return value;
}
/**
