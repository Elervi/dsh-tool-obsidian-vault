...
async function resolveVaultRoot(config, exec, vaultArg) {
    ...
    if (vaultArg && vaultArg.trim().length > 0) {
        ...
        if (!looksAbsolute)
            throw new Error(`未知的 Obsidian 库：${target}（可用 vault_list_vaults 查看，或传绝对路径）`);
        if (!config.allowArbitraryRoots) {
            throw new Error(`未注册的库路径：${target}（默认只允许已发现的库与 vaultRoots；如需放行任意绝对路径，请配置 allowArbitraryRoots: true）`);
        }
        return norm(target);
    }
...
export function registerTools(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (!info)
                throw new Error(`笔记不存在：${rel}`);
            if (info.type !== 'file')
                throw new Error(`路径不是文件：${rel}`);
            try {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (info && info.type !== 'file')
                throw new Error(`路径已存在但不是文件：${rel}`);
            if (info && a.unique) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const info = await fs.stat(target, exec.signal);
            if (!info)
                throw new Error(`笔记不存在：${rel}（如需新建请用 vault_create_note）`);
            if (info.type !== 'file')
                throw new Error(`路径不是文件：${rel}`);
            try {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (!oldInfo)
                throw new Error(`笔记不存在：${oldRel}`);
            if (oldInfo.type !== 'file')
                throw new Error(`路径不是文件：${oldRel}`);
            const newInfo = await fs.stat(newTarget, exec.signal);
            if (newInfo)
                throw new Error(`目标已存在：${newRel}`);
            let content;
            try {
